...
export function CardDetail({ card, isLoading = false }: CardDetailProps) {
    ...
    const flavorText = card.flavor_text
        ?? card.card_faces?.map((face) => face.flavor_text).filter(Boolean).join("\n\n")
        ?? "";
        ...
        <Detail
            ...
            metadata={
                <Detail.Metadata>
                    ...
                    {card.card_faces && !card.mana_cost && (
                        card.card_faces
                    ...
                    {!card.artist && card.card_faces && (
                        card.card_faces
                            .filter((face) => face.artist)
                            .map((face, index) => (
                                <Detail.Metadata.Label key={index} title={`Artist (${face.name})`} text={face.artist!} />
                            ))
                    )}
                    {/* Open */}
                    <Detail.Metadata.Separator />
